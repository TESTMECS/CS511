...
do_join(State, Ref, ChatName) ->
    %% 2. check if the client is already in the chatroom
    case maps:is_key(ChatName, State#cl_st.con_ch) of
        true ->
            %% already in the chatroom, send error response
            {err, State };
        false ->
            %% 3. not in the chatroom, request to join
            whereis(server)!{self(), Ref, join, ChatName},
            %% update state to include the new chatroom 
			%% wait for the response from the chatroom
            receive
                {ChatroomPID, Ref, connect, History} ->
                    %% 8. update the state to include the new chatroom and its history
                    NewConCh = maps:put(ChatName, ChatroomPID, State#cl_st.con_ch),
                    NewState = State#cl_st{con_ch = NewConCh},
					%% 9. send message back to gui. 
                    {History, NewState}
            end	
    end.
...
do_leave(State, Ref, ChatName) ->
	%% 2. check in chatroom with ChatName
	case maps:find(ChatName, State#cl_st.con_ch) of 
		...
		{ok, _} ->
			...
			receive
				{_, Ref, ack_leave} ->
					%% 8. remove chatroom from list of chatrooms. 
					NewConCh = maps:remove(ChatName, State#cl_st.con_ch),	
					NewState = State#cl_st{con_ch = NewConCh},
					%% 9. send message back to GUI 
					{ok, NewState}
			end
...
do_msg_send(State, Ref, ChatName, Message) ->
	%% 2,3
	Room = maps:get(ChatName, State#cl_st.con_ch),
	Room!{self(), Ref, message, Message},
	receive
		%% 5.
		{_, _, ack_msg} -> {{msg_sent, State#cl_st.nick}, State};
		_ -> {error_no_message, State}
    end.
...
do_quit(State, Ref) ->
    ...
    receive
        {_, Ref, ack_quit} ->
			GUI = list_to_atom(State#cl_st.gui),
            whereis(GUI) ! {self(), Ref, ack_quit}
    end,
    {ok_shutdown, State}.
